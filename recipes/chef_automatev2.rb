...
node['chef_software']['automatev2_local_users']&.each do |name, hash|
  iam_user name do
    ...
    api_token kitchen? ? kitchen_create_api_token('test_user') : node['chef_software']['automate_admin_token']
    action :create
  end
end
...
node['chef_software']['automatev2_iam_policies']&.each do |name, hash|
  iam_policy name do
    ...
    api_token kitchen? ? kitchen_create_api_token('test_policy') : node['chef_software']['automate_admin_token']
    action :create
  end
end
